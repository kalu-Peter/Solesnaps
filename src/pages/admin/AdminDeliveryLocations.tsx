...
const AdminDeliveryLocations: React.FC = () => {
  ...
  const fetchLocations = async () => {
    try {
      ...
      const { data: locations, error } =
        await supabaseDb.getDeliveryLocations();
  ...
  const createLocation = async () => {
    try {
      ...
      const { data, error } = await supabaseDb.createDeliveryLocation(
        locationData
      );

      if (error) {
        console.error("Failed to create delivery location:", error.message);
        throw new Error(error.message || "Failed to create delivery location");
      }

      console.log("Created delivery location:", data);
  ...
  const updateLocation = async () => {
    ...
    try {
      ...
      const { data, error } = await supabaseDb.updateDeliveryLocation(
      ...
      console.log("Updated delivery location:", data);
  ...
  const deleteLocation = async () => {
    ...
    try {
      ...
      const { error } = await supabaseDb.deleteDeliveryLocation(
        locationToDelete.id.toString()
      );

      if (error) {
        console.error("Failed to deactivate delivery location:", error.message);
        throw new Error(
          error.message || "Failed to deactivate delivery location"
        );
      }
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedLocation
                ...
                : "Add a new pickup location and delivery cost."}
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid gap-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, shopping_amount: e.target.value })
                }
                placeholder="0.00"
                required
              />
            ...
            <div className="grid gap-2">
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="maintenance" className="text-black">
                    Maintenance
                  </SelectItem>
                </SelectContent>
              </Select>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will deactivate the delivery location for "
              {locationToDelete?.city_name}". The location will be marked as
